#  Adapted from Kevan Hong-Nhan Nguyen's repo
...
class OthelloGame:
    ...
    def _get_valid_moves(self, turn: str) -> list:
        
        places = []
        color = 1 if turn == 'W' else -1
        code = {'.':0, 'W':1, 'B':-1}
        board = []
        for y in self.current_board:
            row = []
            for x in y:
                row.append(code[x])
            board.append(row)
        ...
        mat = np.where(board == color)
        positions = [(x,y) for x,y in zip(mat[0],mat[1])]
        for position in positions:
            for direction in range(1,9):
                p = self._get_direction_valid_moves(board, position, direction, color)
                if not p is None:
                    places.append(p)
    ...
    def _get_direction_valid_moves(self, board: np.ndarray, position: tuple, direction: int, color: int) -> Optional[tuple]:
        
        row_inc, col_inc = direction2row[direction], direction2col[direction]
        x, y = position
        k = 1
        flat = []
        while (x+row_inc*k>=0) and (y+col_inc*k>=0) and (x+row_inc*k<self.rows) and (y+col_inc*k<self.cols):
            flat.append((board[x+row_inc*k, y+col_inc*k], (x+row_inc*k, y+col_inc*k)))
            k += 1
